...
const NODES = [
  { id: "rankings",  angle: -90,  label: "RANKINGS",  isRed: false },
  { id: "content",   angle: -30,  label: "CONTENT",   isRed: true  },
  { id: "leads",     angle:  30,  label: "LEADS",     isRed: false },
  { id: "local",     angle:  90,  label: "LOCAL SEO", isRed: true  },
  { id: "analytics", angle:  150, label: "ANALYTICS", isRed: false },
  { id: "authority", angle: -150, label: "AUTHORITY", isRed: true  },
].map((n) => ({
  ...n,
...
export function SEOHeroIllustration() {
  ...
  return (
    <svg viewBox="0 0 400 400" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full" aria-hidden="true">
      ...
      <circle cx={CX} cy={CY} r="90" fill="url(#seo-bg)" />
      ...
      <motion.g initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }}
        ...
        }).join(" ")} fill="url(#seo-grad)" stroke="rgba(33,150,243,0.55)" strokeWidth="1.5" />
        <circle cx={CX} cy={CY - 50} r="3" fill="#c62828" filter="url(#seo-glow)" />
        <circle cx={CX} cy={CY + 50} r="3" fill="#c62828" filter="url(#seo-glow)" />
        {/* trend arrow up */}
        <path d={`M${CX-14},${CY+10} L${CX-5},${CY+1} L${CX+2},${CY+7} L${CX+14},${CY-10}`}
          stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
        <path d={`M${CX+8},${CY-10} L${CX+14},${CY-10} L${CX+14},${CY-4}`}
          stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" fill="none" />
      </motion.g>
      ...
      {NODES.map((n, i) => (
        <motion.g key={n.id}
          ...
          <rect x={n.x - 44} y={n.y - 15} width="88" height="30" rx="15"
            fill="rgba(10,20,50,0.82)" stroke={n.stroke} strokeWidth="1.4" />
          <text x={n.x} y={n.y + 5} textAnchor="middle" fontSize="10.5" fontWeight="800"
            fill={n.dot} fontFamily="system-ui, sans-serif" letterSpacing="0.07em">{n.label}</text>
          <circle cx={n.x - 40} cy={n.y} r="3" fill={n.dot} filter="url(#seo-glow)" />
        </motion.g>
      ))}
      {NODES.map((n, i) => (
        <motion.circle key={`p-${n.id}`} r="2.8" fill={n.dot} opacity="0.9" filter="url(#seo-glow)"
