"use client";

import { motion, useReducedMotion } from "framer-motion";

const CX = 200;
const CY = 200;
const R  = 122;

const WEB_NODES = [
  { id: "design",  angle: -90,  label: "DESIGN",   isRed: false },
  { id: "seo",     angle: -30,  label: "SEO",       isRed: true  },
  { id: "mobile",  angle:  30,  label: "MOBILE",    isRed: false },
  { id: "speed",   angle:  90,  label: "SPEED",     isRed: true  },
  { id: "cms",     angle:  150, label: "CMS",       isRed: false },
  { id: "launch",  angle: -150, label: "LAUNCH",    isRed: true  },
] as const;

const NODES = WEB_NODES.map((n) => ({
  ...n,
  x: +(CX + R * Math.cos((n.angle * Math.PI) / 180)).toFixed(1),
  y: +(CY + R * Math.sin((n.angle * Math.PI) / 180)).toFixed(1),
  stroke: n.isRed ? "rgba(198,40,40,0.75)" : "rgba(21,101,192,0.65)",
  dot:    n.isRed ? "#f87171"              : "#60a5fa",
}));

export function WebsiteDevHeroIllustration() {
  const reduced = useReducedMotion();

  return (
    <svg
      viewBox="0 0 400 400"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="w-full h-full"
      aria-hidden="true"
    >
      <defs>
        <radialGradient id="wd-bg-glow" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   stopColor="#1565c0" stopOpacity="0.35" />
          <stop offset="100%" stopColor="#1565c0" stopOpacity="0"    />
        </radialGradient>
        <linearGradient id="wd-center-grad" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%"   stopColor="#1e88e5" />
          <stop offset="100%" stopColor="#0d47a1" />
        </linearGradient>
        <filter id="wd-glow">
          <feGaussianBlur stdDeviation="3.5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
      </defs>

      {/* ambient glow */}
      <circle cx={CX} cy={CY} r="90" fill="url(#wd-bg-glow)" />

      {/* orbit ring */}
      <circle
        cx={CX} cy={CY} r={R}
        stroke="rgba(21,101,192,0.16)"
        strokeWidth="1"
        strokeDasharray="6 7"
      />
      {/* inner ring */}
      <circle
        cx={CX} cy={CY} r="68"
        stroke="rgba(21,101,192,0.08)"
        strokeWidth="0.8"
        strokeDasharray="3 9"
      />

      {/* diagonal cross-connects */}
      {[0, 1, 2].map((i) => (
        <motion.line
          key={`cross-${i}`}
          x1={NODES[i].x} y1={NODES[i].y}
          x2={NODES[i + 3].x} y2={NODES[i + 3].y}
          stroke="rgba(21,101,192,0.08)"
          strokeWidth="0.7"
          strokeDasharray="4 6"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={reduced ? {} : { delay: 1.2 + i * 0.1, duration: 0.6 }}
        />
      ))}

      {/* spokes */}
      {NODES.map((node, i) => (
        <motion.path
          key={`spoke-${node.id}`}
          d={`M ${CX} ${CY} L ${node.x} ${node.y}`}
          stroke={node.isRed ? "rgba(198,40,40,0.28)" : "rgba(21,101,192,0.32)"}
          strokeWidth="1"
          strokeDasharray="5 5"
          initial={{ pathLength: 0, opacity: 0 }}
          animate={{ pathLength: 1, opacity: 1 }}
          transition={reduced ? {} : { duration: 0.65, delay: 0.3 + i * 0.07, ease: "easeOut" }}
        />
      ))}

      {/* ── centre browser window ───────────────────────────────── */}
      <motion.g
        initial={{ opacity: 0, scale: 0.6 }}
        animate={{ opacity: 1, scale: 1 }}
        transition={reduced ? {} : { duration: 0.55, type: "spring", stiffness: 220, delay: 0.15 }}
        style={{ transformBox: "fill-box", transformOrigin: "50% 50%" }}
      >
        {/* pulse ring */}
        <motion.circle
          cx={CX} cy={CY} r="54"
          fill="none"
          stroke="rgba(33,150,243,0.22)"
          strokeWidth="1.5"
          animate={reduced ? {} : { scale: [1, 1.18, 1], opacity: [0.5, 0.08, 0.5] }}
          transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }}
          style={{ transformBox: "fill-box", transformOrigin: "50% 50%" }}
        />
        {/* second pulse */}
        <motion.circle
          cx={CX} cy={CY} r="44"
          fill="none"
          stroke="rgba(198,40,40,0.14)"
          strokeWidth="1"
          animate={reduced ? {} : { scale: [1, 1.14, 1], opacity: [0.35, 0.05, 0.35] }}
          transition={{ duration: 4.5, repeat: Infinity, ease: "easeInOut", delay: 1.2 }}
          style={{ transformBox: "fill-box", transformOrigin: "50% 50%" }}
        />
        {/* hex fill */}
        <polygon
          points={[0,1,2,3,4,5].map((i) => {
            const a = (i * 60 - 30) * Math.PI / 180;
            return `${(CX + 38 * Math.cos(a)).toFixed(1)},${(CY + 38 * Math.sin(a)).toFixed(1)}`;
          }).join(" ")}
          fill="url(#wd-center-grad)"
          stroke="rgba(33,150,243,0.55)"
          strokeWidth="1.5"
        />
        {/* accent dots */}
        <circle cx={CX} cy={CY - 50} r="3" fill="#c62828" filter="url(#wd-glow)" />
        <circle cx={CX} cy={CY + 50} r="3" fill="#c62828" filter="url(#wd-glow)" />
        {/* browser icon inside hex */}
        {/* frame */}
        <rect x={CX - 18} y={CY - 14} width="36" height="26" rx="3"
          fill="none" stroke="white" strokeWidth="1.8" />
        {/* title bar */}
        <line x1={CX - 18} y1={CY - 6} x2={CX + 18} y2={CY - 6}
          stroke="white" strokeWidth="1.4" />
        {/* dots */}
        <circle cx={CX - 12} cy={CY - 10} r="1.8" fill="white" opacity="0.9" />
        <circle cx={CX - 7}  cy={CY - 10} r="1.8" fill="white" opacity="0.9" />
        <circle cx={CX - 2}  cy={CY - 10} r="1.8" fill="white" opacity="0.9" />
        {/* content lines */}
        <line x1={CX - 12} y1={CY - 1} x2={CX + 10} y2={CY - 1}
          stroke="rgba(255,255,255,0.6)" strokeWidth="1.4" />
        <line x1={CX - 12} y1={CY + 4} x2={CX + 5}  y2={CY + 4}
          stroke="rgba(255,255,255,0.4)" strokeWidth="1.4" />
        <line x1={CX - 12} y1={CY + 8} x2={CX + 8}  y2={CY + 8}
          stroke="rgba(255,255,255,0.3)" strokeWidth="1.4" />
      </motion.g>

      {/* ── nodes ──────────────────────────────────────────────── */}
      {NODES.map((node, i) => (
        <motion.g
          key={`node-${node.id}`}
          initial={{ opacity: 0, scale: 0.5 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={reduced ? {} : { duration: 0.45, delay: 0.55 + i * 0.09, type: "spring", stiffness: 260 }}
          style={{ transformBox: "fill-box", transformOrigin: `${node.x}px ${node.y}px` }}
        >
          <rect
            x={node.x - 38} y={node.y - 15}
            width="76" height="30" rx="15"
            fill="rgba(10,20,50,0.82)"
            stroke={node.stroke}
            strokeWidth="1.4"
          />
          <text
            x={node.x} y={node.y + 5}
            textAnchor="middle"
            fontSize="11"
            fontWeight="800"
            fill={node.dot}
            fontFamily="system-ui, -apple-system, sans-serif"
            letterSpacing="0.08em"
          >{node.label}</text>
          <circle
            cx={node.x - 34} cy={node.y}
            r="3" fill={node.dot}
            filter="url(#wd-glow)"
          />
        </motion.g>
      ))}

      {/* ── travelling data-flow dots ───────────────────────────── */}
      {NODES.map((node, i) => (
        <motion.circle
          key={`pulse-${node.id}`}
          r="2.8"
          fill={node.dot}
          opacity="0.9"
          filter="url(#wd-glow)"
          animate={reduced ? {} : {
            x: [CX, node.x],
            y: [CY, node.y],
            opacity: [0, 0.9, 0],
            scale:   [0.4, 1.1, 0.4],
          }}
          transition={{
            duration: 1.7,
            delay: 1.4 + i * 0.28,
            repeat: Infinity,
            repeatDelay: 2.3 + i * 0.3,
            ease: "easeInOut",
          }}
        />
      ))}
    </svg>
  );
}
